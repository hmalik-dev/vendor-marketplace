...
- `messages` is `FORCE`d and has policies for the sender (insert), the counterparty (mark read), a participant (select) and admins (`app.role = 'admin'` **and** `app.admin = 'true'`, which only the console's own read paths set; select only). Identity comes from `withRequestIdentity` (`packages/db/src/request-identity.ts`), which sets `app.user_id` and `app.role` transaction-locally.

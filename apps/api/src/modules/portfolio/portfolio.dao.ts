...
export async function deletePortfolioItemById(
  ...
  const deleted = await db
    ...
    .returning({ imageUrl: portfolioItems.imageUrl, thumbnailUrl: portfolioItems.thumbnailUrl });

  const row = deleted[0];
  return row ? { imageUrl: row.imageUrl, thumbnailUrl: row.thumbnailUrl } : null;
}
...
/**
 * Writes the new order, in one transaction so a failure part-way leaves the
 * previous order intact.
 *
 * The profile's cover is a separate, direct upload (`vendor-cover`) and is never
 * written from here: a reorder or delete that rewrote it would silently replace
 * the photo the vendor chose on purpose.
 */
 ...
 * more than one of them — a vendor may point `vendor_profiles.cover_image_url`
 * at a key a portfolio item also holds, so the cover can be a *second*
 * reference to a photo rather than an upload of its own, and

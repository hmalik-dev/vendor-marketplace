...
import { alertNow } from './admin-alerts.service.js';
import { adminLocalTime, runAdminDigest, type AdminDigestDeps } from './admin-digest.service.js';
...
describe('the admin digest (VEN-405)', () => {
  let harness: TestHarness;
  const logLines: string[] = [];

  function deps(overrides: Partial<AdminDigestDeps> = {}): AdminDigestDeps {
    return {
  ...
  it("reads the admin's date and hour in their own zone", () => {
    expect(adminLocalTime(NOW, 'America/New_York')).toEqual({ date: '2026-09-14', hour: 8 });
    expect(adminLocalTime(new Date('2026-09-14T02:00:00Z'), 'America/New_York')).toEqual({
      date: '2026-09-13',
      hour: 22,
    });
    expect(() => adminLocalTime(NOW, 'Mars/Olympus_Mons')).toThrow(RangeError);
  });

  it('is not due before 07:00 local time', async () => {
    await seedActivity();

    expect(await runAdminDigest(deps(), new Date('2026-09-14T10:59:00Z'))).toBe('not-due');
    expect(harness.email.sent).toEqual([]);
  });

  it('sends on an empty day too, so its absence means the timer stopped', async () => {
    expect(await runAdminDigest(deps(), NOW)).toBe('sent');
    ...
    await seedActivity();
    expect(await runAdminDigest(deps(), new Date(NOW.getTime() + 4 * HOUR))).toBe(
      'already-claimed',
    );
    expect(harness.email.sent).toHaveLength(1);
  });
  ...
  it('sends one digest whose totals equal the seeded rows', async () => {
    ...
    expect(await runAdminDigest(deps(), NOW)).toBe('sent');
    expect(await runAdminDigest(deps(), new Date(NOW.getTime() + HOUR))).toBe('already-claimed');
  ...
  it('gives the claim back when the send fails, so a later tick retries', async () => {
    ...
    expect(await runAdminDigest(deps(), NOW)).toBe('failed');
  ...
  it('logs rather than sends when no admin address is configured', async () => {
    await seedActivity();

    expect(await runAdminDigest(deps({ to: undefined }), NOW)).toBe('logged');
    expect(

...
try {
  const summary = await reconcileClerkUsers(db, clerk.users);

  process.stdout.write(
    `Reconciled ${summary.examined} user(s) against Clerk\n` +
      `  ${summary.updated} corrected\n` +
      `  ${summary.deleted} retired (deleted in Clerk)\n` +

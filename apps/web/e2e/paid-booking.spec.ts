...
/* VEN-779 DIAGNOSTIC — removed before merge. */
function recordTraffic(page: Page, label: string, lines: string[]): void {
  const t0 = Date.now();
  const stamp = (): string => `${new Date().toISOString()} +${Date.now() - t0}ms [${label}]`;
  page.on('request', (r) => {
    const h = r.headers();
    lines.push(
      `${stamp()} REQ ${r.method()} ${r.url()} rsc=${h.rsc ?? '-'} prefetch=${h['next-router-prefetch'] ?? '-'} action=${h['next-action'] ?? '-'} auth=${h.authorization ? 'bearer' : '-'}`,
    );
  });
  page.on('response', (r) =>
    lines.push(`${stamp()} RES ${r.status()} ${r.request().method()} ${r.url()}`),
  );
  page.on('requestfailed', (r) =>
    lines.push(`${stamp()} FAIL ${r.method()} ${r.url()} ${r.failure()?.errorText}`),
  );
  page.on('console', (m) => lines.push(`${stamp()} CONSOLE ${m.type()} ${m.text()}`));
  page.on('pageerror', (e) => lines.push(`${stamp()} PAGEERROR ${e.message}`));
  page.on('framenavigated', (f) => {
    if (f === page.mainFrame()) lines.push(`${stamp()} NAV ${f.url()}`);
  });
}
...
for (const round of [1, 2, 3, 4, 5, 6, 7, 8]) {
  test.describe(`paid booking round ${round}`, () => {
    ...
    test.beforeEach(({ customerPage, vendorPage }) => {
      traffic.length = 0;
      recordTraffic(customerPage, 'customer', traffic);
      recordTraffic(vendorPage, 'vendor', traffic);
    });
    test.afterEach(async ({}, testInfo) => {
      await testInfo.attach('traffic', { body: traffic.join('\n'), contentType: 'text/plain' });
    });
    ...
    if (round === 1)
      test('a declined card says so and leaves the booking unpaid', async ({
        ...
        const decline = customerPage
          .getByRole('alert')
          .filter({ hasText: 'Your card was declined' });
    ...
    if (round === 1)
      test('after the event the vendor completes it and the customer’s review reaches the storefront', async ({
        ...
        try {
          ...
          recordTraffic(page, 'visitor', traffic);

...
| Photos and files     | Neon Object Storage | Vendor covers and portfolio images.                        |
...
**One exception, and it is the reason reporting works at all: when a message thread is reported, our admins can read it.** They can read that thread, and only that thread, and only while the report is open — a closed report stops being a key to the conversation. Every such read is written to an internal log naming the admin, the thread and the report it was read under. Staff never write into a thread: an admin reads and then acts, and nothing you see in a conversation is from us. We do not otherwise read your messages, and there is no way in the product for anybody to browse conversations they have no report about.

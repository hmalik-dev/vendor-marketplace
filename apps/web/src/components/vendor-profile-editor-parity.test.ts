...
describe('the section rail matches frame 09’s nav (#360)', () => {
  /**
   * The rail block, ending at the rail's own closing tag.
   *
   * Every row closes on its own line, so the rail's `</div>` is the first one
   * that opens a line. The 2026-09-25 resync removed the legend this slice used
   * to end on; that needle failed silently once already — `indexOf` returned
   * -1, the slice stopped truncating, and every assertion below still passed —
   * so the end is asserted rather than defaulted: a slice that cannot fail is
   * not a slice.
   */
  const RAIL_END = '\n</div>';
  ...
  function railBlock(): string {
    ...
    const end = side.indexOf(RAIL_END);

    expect(end, 'the rail closes').toBeGreaterThan(-1);

    return side.slice(0, end);
  }

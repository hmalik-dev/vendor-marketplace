...
| 13 | 13 Admin | `/admin/vendors?status=review` | admin | 1440x900 | **PASS (4 rulings owed)** | #15 |
...
| PB2-S4 | Access | Focus ring on every interactive element | **PARTLY SELF-CLOSED, then PASS 2026-08-29 (#120, `33e03db`+`16a5426`)** — re-measuring found the `Messages`/`Dashboard` links already correct: a global `:focus-visible` rule landed between the sweep and this lane. The auth provider's `Open user menu` had not, and drew a single 4px clay at 50% with no offset ring |

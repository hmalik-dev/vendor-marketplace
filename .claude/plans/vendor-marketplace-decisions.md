...
>
> **Superseded 2026-09-20 by VEN-455**: uploaded images now live on Neon Object Storage, not Cloudflare R2.
...
**This closes the one thing D33 deferred.** #395 found the same `/30` on the `Button` primitive from the checkout screen and declined to settle it there — *"the law file and the primitive disagree everywhere, not on checkout, so it belongs to whichever ticket owns the `Button` primitive"*. Right call: the disagreement was in nine places, and the `Button` was one of them.

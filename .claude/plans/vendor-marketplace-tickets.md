...
**Critical path:** #0 → #1 → #2 → #3 → #17 → #21 → #4 → #6a → #23 → #24 → #6c ✅ →
**#31 ◄── BUILD NEXT** → #28 → #29 → #6b → #16 → #7 → #22b → #22a → #8 → #9 → #10 → #30 → #19 → #20 → #12 → #11 → #14 → #15

```
PHASE 0-1: Foundation  ✅ complete
  #0 → #1 → #2 → #3 → #17 → #21 → #4 → #6a → #23 → #6c → #24 → #18

PHASE 2: Repair what is already serving users        ◄── you are here
  #31 Shipped-Surface Defect Sweep      (no blockers — start today)
  #28 Application States Foundation     frames 15, 16, 26
   └► #29 States Retrofit               frames 17, 18, 24, 25

PHASE 3: Close the funnel
  #28 ─┬► #6b Public Vendor Profile     frame 03   (unblocks everything)
       │    └► #7 Booking Request     frames 04 + 22   (API + screen)
       │        ├► #22b Bookings Hub    frames 07 + 19   (also needs #16)
       │        ├► #22a Vendor Dash     frames 08 + 20
       │        ├► #8  Messaging        frames 10 + 23
       │        └► #10 Payment          frames 05, 06 + 21   (needs #9)
       └► #16 Customer Profile          (parallel-safe)

PHASE 4: Design parity debt   #25 · #26   (parallel-safe throughout)
...
## The credential-free batch — what can start today

Verified against `pnpm preflight` on 2026-08-27: **Stripe, Resend and Sentry keys are
all still placeholders**, so #9, #10, #11 and #15 cannot start, and #12 / #14 inherit
that block through #10. #19 and #20 need external accounts and a human.

These need nothing beyond the credentials already present (`core`, `auth`, `storage`,
`e2e`), and they run in this order:

| Order | # | Frames | Why here |
| --- | --- | --- | --- |
| 1 | **#31** | — | Zero blockers, all defects, all in shipped code. Cheapest possible win |
| 2 | **#28** | `15` `16` `26` | The state vocabulary every later screen composes from |
| 3 | **#29** | `17` `18` `24 Image upload` `25 Upload failures` | Retrofits #28 onto the three shipped surfaces that need it |
| 4 | **#6b** | `03` | Unblocks the whole booking funnel; kills the 404 every card leads to |
| 5 | **#16** | — | No frame; unblocks #22b. Parallel-safe |
| 6 | **#7** | `04` `22` | The spine — API, state machine and the screen that drives them |
| 7 | **#22b** | `07` `19` | |
| 8 | **#22a** | `08` `20` | |
| 9 | **#8** | `10` `23` | Closes the loop without touching Stripe |
| — | **#25 #26 #30** | `02` | Parallel-safe at any point |
...
**Milestone:** M3 | **Priority:** P0 Critical | **Status:** Backlog | **Capabilities:** `core` `auth` `storage`
...
**Milestone:** M1.8 | **Priority:** P0 Critical | **Status:** Backlog | **Capabilities:** `core` `auth`
...
**Milestone:** M2 | **Priority:** P1 High | **Status:** In Progress | **Capabilities:** `core` `auth` `storage`
...
**Milestone:** M3 | **Priority:** P1 High | **Status:** Backlog | **Capabilities:** `core` `auth` `storage`
...
**Milestone:** M6 | **Priority:** P2 Medium | **Status:** Backlog | **Capabilities:** `core` `auth` `email`
...
**Milestone:** M4.5 | **Priority:** P0 Critical | **Status:** Backlog | **Capabilities:** all
...
**Milestone:** M4.5 | **Priority:** P0 Critical | **Status:** Backlog | **Capabilities:** `core` `storage`
**Blocked by:** #18

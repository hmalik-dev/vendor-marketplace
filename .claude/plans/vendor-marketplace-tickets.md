...
| **280** | **`nearby-availability` has a real-clock dependency, so the API suite goes red on a date CI never saw** | P1 | M3 | P1 High | Backlog | — | None | `core` | **Found 2026-08-29 by lane 82 while running the gate.** `apps/api/src/modules/vendors/nearby-availability.routes.test.ts:184` — _never suggests a past date when the wanted date is today_ — expects `2026-08-31`, receives `2026-08-29`. **Reproduced on a clean `main` at `32d1656` with an empty working tree**, so no lane's changes cause it. CI passed on that exact SHA at 22:44 UTC and the same test fails locally hours later — the signature of a test reading the real clock, which `.claude/rules/testing.md` forbids outright. Until it is fixed `pnpm test` is red for **every** lane, so the pre-commit gate cannot be read honestly and a real regression can hide behind it. Fix by injecting the clock (fake timers or a seeded `today`), never by widening the assertion. Use `/debug-flaky-test`. |

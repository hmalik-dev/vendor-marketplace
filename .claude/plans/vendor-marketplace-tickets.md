...
Rows are ordered by build sequence, not by ticket number. **Recounted programmatically 2026-09-07 after #425 landed and its row was deleted: 4 rows — 2 Backlog and 2 `Deferred — needs a human`.** Startable now: **#427** alone. It was waiting on #423, which has landed and owns the release date, the `disputed` hold and the payout constant it reads; #424 built the vendor-facing surface that reports them and #425 the customer's way into the hold. **#427 is therefore the whole of what an unattended run can pick up**: #428 built the footer's legal row, so that ticket adds the three pages the row already links to, and until it lands `/terms`, `/privacy` and `/cookies` are 404s reachable from every page. #370 waits on #362; #362 and #374 need the account holder, so the board is one ticket away from needing one too. **Do not hand-maintain this number, recount it.**

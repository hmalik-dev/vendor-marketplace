...
| **67** | **`POST /booking-requests` has no idempotency — 3 clicks created 3 bookings** | **P1** | **M3** | **P0 Critical** | **In Progress** | `worktree-67` | **None** | `core` `auth` | **Adversarial sweep 2026-08-28.** Server-side; UI guard only covers a physical double-click. Highest-severity finding. **PR [#10](https://github.com/hmalik-dev/vendor-marketplace/pull/10) open with squash auto-merge enabled, CI green on `cff8455`** — waiting because the branch is `BEHIND` main. Branch is `worktree-67`, **not** the `lane/67` the lane manifest records |

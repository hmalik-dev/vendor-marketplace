...
| **251** | **`-webkit-font-smoothing: antialiased` is applied app-wide, which `01-foundations.md` explicitly forbids** | P1 | M3 | P1 High | Backlog | — | None | `core` | **Found by lane 82's final parity pass 2026-08-29, confirmed from source.** `01-foundations.md:189` says verbatim: *"Do not add `-webkit-font-smoothing: antialiased`. The frames do not set it. Applying it to the app alone changes glyph weight and fails the font axis against every frame. If it is ever wanted, it goes in `Orla - Screens.dc.html` and the app in the same change, or in neither."* The app sets it in two places: `apps/web/src/app/layout.tsx:99` and `apps/web/src/app/global-error.tsx:31` (`<body className="… antialiased">`), while the frame computes `auto`. Axis **Font**, and it is **app-wide, not landing-only** — every screen's font axis is affected, so this is worth doing before further parity tickets. The prohibition names the only two acceptable end states, so the fix is to remove it from the app (not to add it to the frames, which would edit `design/`).  **Renumbered from #241** on merge — lane 90 filed a different #241. |
...
| **253** | **01 Landing — hero Search button carries an 8px left margin the frame does not, narrowing all three segments** | P1 | M3 | P2 Medium | Backlog | — | None | `core` | **Found by lane 82's final parity pass 2026-08-29 and independently re-measured.** Frame `01 Landing` gives the Search submit `margin-left: 0`; `search-bar.tsx` gives it `sm:ml-2` (8px). The bar's total width matches (727.594px in both) so the 8px is absorbed by the three flex segments: **Vendor type 229.97 vs the frame's 233.33, City 194.91 vs 197.48, Event date 159.52 vs 161.58.** The button's own box is already exact (102.203x44) — #84 closed that — so this is the segments, not the button. Axis **Layout**. Not fixed by lane 82: #84 was scoped to the button's box and padding, and an un-ticketed defect gets filed rather than folded into a closed ticket. Likely a one-class change once someone confirms the frame's segments are the intent.  **Renumbered from #243** on merge — lane 90 filed a different #243. |

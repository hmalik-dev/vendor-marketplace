...
**All but one are cleared.** Updated 2026-08-29.

| # | Ticket | What only you can do | Where |
| --- | --- | --- | --- |
| 1 | #206 | Enter payment details, select **Launch** | Neon console -> Billing |

Everything else is done: local dev on Docker Postgres 18 (#200), the Neon
`staging` branch and Railway `staging` environment (#203), the `production`
branch with protection and the Vercel branch flip (#202), per-PR Neon branches
proven on a real PR (#205). #201 closed without work and #204 closed by handing
migrations to Railway's `preDeployCommand`.

Once Launch is active, these become agent-executable via the Neon MCP:
protected branch on `production`, history window 6h -> 7 days, a scheduled
backup, and a spending notification.
...
**Milestone:** M3 | **Priority:** P0 Critical | **Status:** Backlog | **Capabilities:** `core` `auth`
**Blocked by:** None

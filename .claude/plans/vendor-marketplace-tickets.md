...
**Blocked by:** **None** — #423 landed 2026-09-07 (`150fe77`).
...
units. Every number here comes from columns that ticket created, and they now
exist.

#### What #423 leaves you, and the one thing that changed meaning

`GET /vendor/dashboard` already answers `nextPayout` with all three facts this
surface needs, so **no new API read is required** — only the rendering:

- `vendorPayoutCents` — the stored figure, never a recomputed fee;
- `releaseAt` — a real `Date`, from `payoutReleaseAt(eventDate)`, the same
  helper the payout sweep pays on, so the date shown cannot drift from the date
  paid;
- `status` — `pending` | `held` | `released`, from `payoutStatusOf`. **Read this
  rather than `booking.status`**: acceptance 16 exists precisely so no surface
  has to know which member of `BOOKING_STATUSES` means the money is stuck.

**`vendor_payout_cents` no longer means "what was agreed". It means "what is
still owed" (D37).** A cancellation before the payout is released rewrites it to
the share the vendor keeps — half at D3's 48-hour tier, zero on a full refund.
That is deliberate: under separate charges nobody holds the vendor's retained
share but Orla, so it has to be written down or the platform silently keeps it.
Two consequences for this surface: **the figure can go down**, and **a cancelled
booking carrying a residual is still a real pending payout**, so do not assume a
payout line belongs only to a live booking.

One trap, already paid for once and now in
`.claude/rules/web-route-boundaries.md`: `releaseAt` is a `z.date()`, so it needs
its `z.coerce.date()` in `wire-schemas.ts`. That one is already there — but
**exercise this screen with a fixture that actually has a pending payout.** The
dashboard 500'd for exactly the vendors who were owed one and rendered perfectly
for everyone else, which is how it passed a browser pass looking clean.
...
**Blocked by:** **None** — #423 landed 2026-09-07 (`150fe77`); it owns the `disputed` hold this report places, and the release window a report has to land inside.
...
| Payout interval | **72 hours after the event date** — `PAYOUT_RELEASE_HOURS`, one constant, three render sites | **D35** |
...
- ~~**`PAYOUT_RELEASE_HOURS` itself.**~~ **RULED 2026-09-06 as D35: `72` hours**
  ...
  its own. The four-terms panel's `Event + 2 days` becomes the value D35 sets.

...
**Milestone:** M4.5 | **Priority:** P0 Critical | **Status:** Deferred — needs a human | **Capabilities:** `stripe`
**Blocked by:** A ruling on who bears the cost of a cancellation
...
- [ ] A gateway test asserting the flag pair sent for a destination charge
- [ ] A test that the fake rejects `refund_application_fee` without
      `reverse_transfer`, and that the cancellation tests still pass against it
- [ ] A driven cancellation against Stripe test mode, refunding a real intent

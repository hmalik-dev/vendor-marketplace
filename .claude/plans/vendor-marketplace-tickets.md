...
Rows are ordered by build sequence, not by ticket number. **28 rows — 16 Backlog, 1 In Progress, 8 Deferred, 2 Blocked, and #14 Done pending the next archive sweep.** Recounted programmatically from this table on 2026-08-30, after #329 closed, #336–#339 were filed, and #11 and #14 were corrected. **13 tickets are workable** — #11, #300, #302, #305, #322, #323, #326, #332, #333, #334, #336, #337, #338. Two more are `Backlog` but gated: **#20** on #19 and **#15** on #14, so a Backlog count is not a ready count — read `Blocked By`, and trust `pnpm preflight --ticket <n>` over it, since #14 read `None` while its gate was failing on `SENTRY_DSN`. Of the workable set, **#335 unblocks three of the deferred rows in one sitting** and is the highest-leverage thing on the board.

...
- [Every `FORBIDDEN` is read as a suspension](every-forbidden-is-read-as-a-suspension.md) — `useApi` keys on `ACCOUNT_SUSPENDED`; the server twin does not; VEN-763 role detail clean
...
- [Route-landing sweep strictness sources](route-landing-sweep-strictness-sources.md) — detector only tightens; VEN-757 resolves `src/lib` wrapper gates (PASS)

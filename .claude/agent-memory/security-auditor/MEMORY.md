...
- [Every `FORBIDDEN` is read as a suspension](every-forbidden-is-read-as-a-suspension.md) — `useApi` now keys on `ACCOUNT_SUSPENDED` (VEN-701 `NAME_REQUIRED` safe there); the server twin still reads any 403 as banned
...
- [`metadata.env` is the cross-deployment filter](env-tag-is-the-cross-deployment-filter.md) — VEN-529: one shared test account, a branched DB, so the intent tag is the only tier signal; filter order is load-bearing and the retrieve can 500 an alert path; VEN-644 balance check reads the shared balance; VEN-645 transfer/payout/EFW branches clean

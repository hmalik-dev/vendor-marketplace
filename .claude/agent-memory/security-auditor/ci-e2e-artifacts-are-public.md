...
VEN-779 `e2e/request-log.ts` attaches a role page's origin+path log on failure (PASS): query/hash/userinfo dropped, no headers or bodies. Residue is path-borne tokens only (Stripe login/account links `connect.stripe.com/.../<token>`, a Better Auth `/api/auth/reset-password/<token>`), which no role-page spec traverses and which are single-use and spent by the navigation. Before that change, `rateLimited` in `fixtures.ts` already put a 429's full URL (query included) into an annotation — low, since a stream ticket is short-lived.

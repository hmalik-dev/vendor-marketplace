...
provider-deleted identity frees its address _in auth_ while the local row keeps it:

- **Re-registration is a hard lockout.** Same person, new the auth provider id, same
  address: `insertUserIfAbsent` is declined by `users_email_key`, finds no row
  under the new the auth provider id, and (since #442) **throws**. The caller sees an opaque
  ...
  "two identities that cannot co-exist" in the auth provider, which the retired row makes
  false, and a message pointing an operator at "another account" that is really
  the caller's own closed one.
- **`user.updated` cannot mirror onto a taken address.** `updateUserByAuthId`
...
`auth_user_id` and the update's `where` is `auth_user_id = X AND deleted_at IS
...
as an unhandled throw. Any diff touching account closure, the auth provider mirror, or

...
- No Mailosaur needed for a payable request (VEN-721): E2E customer `/vendors/e2e-test-studio/request` (buttons matched by `button:has-text(...)`, not role name; location input `input[id$=eventLocation]`) -> custom request has no package, so vendor dashboard offers only "Send quote" (fill `input[id^=quote-<reqid>]`, click the `^Send$` button in its container) -> customer /bookings/<id> "Accept quote" -> /checkout. Decline card 4000000000000002 flips the button to "Try this payment again"; refill the iframe card then click it. Admin refund confirm is unreachable: a booking-linked case needs a held post-event payout, /support is email-only.

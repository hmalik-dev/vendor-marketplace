# Soundtrack licence receipt — "Introducing Orla"
...
Use this verbatim (the artist's own template, with the track title filled in):

```
Home Was You by Scott Buckley – released under CC-BY 4.0. www.scottbuckley.com.au
```

Place it in the video description / page credits — for YouTube the artist requires the credit be **in the description, not burned into the video**, or the upload will draw an automated copyright claim (release process: https://www.scottbuckley.com.au/library/copyright-claims-release/).

## Caveat to check before running this as a paid ad

CC BY 4.0 covers paid advertising **only where the credit line can be displayed**. Ad formats that strip captions or descriptions leave no place for it. For those placements the artist's Web Licence is **$40 AUD** for this track, one-off, per project — buy it at https://www.scottbuckley.com.au/library/licensing/ rather than run the ad uncredited.

## Other restrictions that apply

- Do not submit the finished film's audio to YouTube Content ID, Facebook Rights Manager, or any music distributor (DistroKid, CD Baby, etc.). This is the artist's explicit condition, and exists to stop false claims against everyone else using the library.
- The music may not be resold on its own, only synchronised with other media.

...
/** The registry's shape for `WEB_TIER_KEY`; a lane file holding anything else is rewritten. */
export const LANE_WEB_TIER_KEY_SHAPE = /^[A-Za-z0-9_-]{32,}$/;

export function renderLaneEnv(
  manifest: LaneManifest,
  databaseUrl: string,
  storage: LaneStorageEnv,
  webTierKey: string,
): string {
  return [
     ...
     */
    /*
     * The lane's own storage branch (`lane-<ticket>`), never a shared bucket.
     * Written unconditionally: a lane file with no storage rows would fall back
     * to the registry's local defaults, which name no bucket at all now that
     * there is no emulator.
     */
    ...LANE_STORAGE_KEYS.map((key) => `${key}=${storage[key]}`),
    /*
     * The lane's own web-tier key, shared by its web and API processes. A
     * sign-up needs it: the auth proxy records the role chosen on the form at
     * the API's internal route, and refuses the sign-up when it cannot
     * (VEN-662). Local to this file, so no two lanes share one.
     */
    `WEB_TIER_KEY=${webTierKey}`,
    '',
  ].join('\n');
}

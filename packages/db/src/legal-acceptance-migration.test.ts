...
/**
 * `0030` against a database that already holds acceptances — acceptance 3.
 *
 ...
 * can damage, and the two columns `0030` adds are `NOT NULL` on a table that is
 ...
 *
 * So this runs the migrations in journal order, stops before `0030`, writes a
 * row in the shape `0029` accepted, and only then applies `0030`.
 *
 ...
 */
const MIGRATION_0030 = '0030_overjoyed_barracuda';

/**
 * The hash `0030` backfills.
 *
...
beforeAll(async () => {
  ...
  await testDb.migrateUpTo(MIGRATION_0030);
});
...
describe('0030 against a database that already holds acceptances', () => {
  it('leaves the existing acceptance readable, with the columns it was written with', async () => {
    ...
    await applyMigration(MIGRATION_0030);
  ...
  /** And the migrated row is still protected, by the rule `0030` replaced. */

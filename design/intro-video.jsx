...
const MOTION = {
  enter: (from, to, start, dur) =>
    animate({ from, to, start, end: start + (dur || 0.7), ease: Easing.easeOutCubic }),
  draw: (from, to, start, dur) =>
    animate({ from, to, start, end: start + (dur || 1.2), ease: Easing.easeInOutCubic }),
  pop: (from, to, start, dur) =>
    animate({ from, to, start, end: start + (dur || 0.55), ease: Easing.easeOutBack }),
};
...
const DW = 1440,
  DH = 900,
  CT = 40; // device box, chrome height
    ...
    <div style={{ position: 'relative', width: size + g, height: size, ...style }}>
      ...
      <div
        style={{
          ...
          height: size,
          borderRadius: '50%',
          border: Math.max(1.2, size * 0.075) + 'px solid ' + ring,
...
const Btn = ({ children, kind = 'p', style }) => (
  <span
    style={{
      ...
      whiteSpace: 'nowrap',
      background: kind === 'p' ? C.clay : C.s0,
      color: kind === 'p' ? C.s0 : C.s900,
      border: kind === 'p' ? '1px solid ' + C.clay : '1px solid ' + C.s300,
      ...style,
    }}
  >
    {children}
...
const Ph = ({ h, label, style }) => (
  <div
    style={{
      height: h,
      background: 'repeating-linear-gradient(135deg,#E6DFD3 0 9px,#EFE9DF 9px 18px)',
      display: 'flex',
      alignItems: 'flex-end',
      padding: 10,
      boxSizing: 'border-box',
  ...
  >
    <span style={{ font: '400 10px ' + MONO, color: C.s600 }}>{label}</span>
  </div>
);
const COVER_TONES = [
  ['#C9744C', '#8E3F20'],
  ...
  return (
    <div
      style={{
        ...
        height: h,
        overflow: 'hidden',
        background: 'linear-gradient(152deg,' + t[0] + ' 0%,' + t[1] + ' 100%)',
        ...style,
      }}
    >
      <div
        style={{
          ...
          borderRadius: '50%',
          background:
            'radial-gradient(circle at 42% 38%, rgba(255,253,249,.22), rgba(255,253,249,0) 68%)',
        }}
      />
      <div
        style={{
          ...
          borderRadius: '50%',
          background:
            'radial-gradient(circle at 50% 50%, rgba(35,32,28,.26), rgba(35,32,28,0) 70%)',
        }}
      />
      <div
        style={{
          ...
          paddingBottom: '44%',
          borderRadius: '50%',
          border: '1px solid rgba(255,253,249,.30)',
        }}
      />
      <div
        style={{
          ...
          bottom: 0,
          height: '46%',
          background: 'linear-gradient(to top, rgba(35,32,28,.30), rgba(35,32,28,0))',
        }}
      />
    </div>
  );
    ...
    <div style={{ position: 'relative', height: h, overflow: 'hidden', ...style }}>
      ...
      <div
        style={{
          ...
          bottom: 0,
          height: '42%',
          background: 'linear-gradient(to top, rgba(35,32,28,.28), rgba(35,32,28,0))',
        }}
      />
    </div>
  );
...
const Avatar = ({ t = 'AN', sage, size = 32 }) => (
  <div
    style={{
      ...
      height: size,
      borderRadius: '50%',
      background: sage ? C.sage100 : C.clay100,
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      font: '400 ' + Math.round(size * 0.42) + 'px ' + SERIF,
      color: sage ? C.sageT : C.clayD,
    }}
  >
    {t}
  </div>
...
function Opening({ T, CUES }) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          font: '400 112px ' + SERIF,
          color: C.s900,
          lineHeight: 1,
          opacity: wordO,
          transform: 'translateY(' + wordX + 'px)',
          letterSpacing: '-.02em',
        }}
      >
        Orla
...
function FragCard({ f, T, CUES }) {
  ...
  const body = {
    tabs: [
      ['15 open tabs', C.s900],
      ['photographers austin', C.s600],
      ['austin wedding photo prices', C.s600],
      ['is $1,400 too much…', C.s600],
    ],
    dm: [
      ['Instagram DM', C.s900],
      ['hi! are you free June 14?', C.s600],
      ['seen · 3 days ago', C.s400],
    ],
    sheet: [
      ['vendors.xlsx', C.s900],
      ['col B — "maybe?"', C.s600],
      ['col C — no price', C.s600],
      ['col D — ???', C.s400],
    ],
    dm2: [
      ['Text message', C.s900],
      ['who was the caterer again', C.s600],
      ['delivered', C.s400],
    ],
    mail: [
      ['Inbox (4)', C.s900],
      ['Re: Re: Re: availability', C.s600],
      ['deposit — how much?', C.s600],
    ],
  }[f.kind];
  return (
    <div
      style={{
        ...
        transform:
          ...
          f.r +
          'deg) scale(' +
          (0.9 + 0.1 * inn) * (0.7 + 0.3 * collapse) +
          ')',
        opacity: inn * collapse,
        background: C.s0,
        borderRadius: 12,
        border: '1px solid ' + C.s300,
        boxShadow: '0 12px 40px rgba(35,40,38,.16)',
        padding: '14px 16px',
      }}
    >
      {body.map((b, i) => (
        <div
          key={i}
          style={{
            font: (i === 0 ? '600 14px ' : '400 13px ') + SANS,
            color: b[1],
            marginTop: i ? 7 : 0,
          }}
        >
    ...
    >
      <div
        style={{
          ...
          height: 140 + halo * 700,
          borderRadius: '50%',
          border: '1px solid rgba(180,85,47,' + 0.4 * (1 - halo) + ')',
        }}
      />
      <div
        style={{
          ...
          height: 140 + halo * 380,
          borderRadius: '50%',
          border: '1px solid rgba(35,32,28,' + 0.22 * (1 - halo) + ')',
        }}
      />
...
function SearchScreen({ T, CUES }) {
  ...
  const seg = (label, value, on, w) => (
    <div style={{ width: w, padding: '0 22px', borderRight: '1px solid ' + C.s200 }}>
      ...
      <div style={{ font: '500 17px ' + SANS, color: on ? C.s900 : C.s400, marginTop: 3 }}>
        {on ? value : 'Any'}
      </div>
    </div>
  );
  const cats = [
    ...
    ['Music', 'DJs, bands, hosts'],
    ['Catering', 'Food, bar, carts'],
    ['Venues', 'Halls & outdoor'],
    ['Florals', 'Bouquets & decor'],
    ['Beauty', 'Hair & makeup'],
  ];
  return (
    <div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 34 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <Mark size={21} gap={9} />
            <span style={{ font: '400 25px ' + SERIF, color: C.s900 }}>Orla</span>
          </div>
          <div style={{ display: 'flex', gap: 24, font: '500 14px ' + SANS, color: C.s700 }}>
            <span>Browse</span>
      ...
      <div
        style={{
          ...
          top: 60,
          bottom: 0,
          background: 'linear-gradient(155deg,#F8F5EF 0%,#F7F0E8 52%,#F2E4D8 100%)',
          overflow: 'hidden',
        }}
      >
        <div
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, top: 104, textAlign: 'center' }}>
        ...
        >
          <span style={{ width: 5, height: 5, borderRadius: '50%', background: C.clay }} />
          Now booking in Austin
        </div>
        <div
          style={{
            font: '400 58px ' + SERIF,
        ...
        >
          Book your vendors
          <br />
          <em style={{ fontStyle: 'italic', color: C.clayT }}>without the back-and-forth.</em>
        </div>
        <div
          style={{
      ...
      <div
        style={{
          ...
          border: '1px solid ' + C.s300,
          borderRadius: 999,
          boxShadow: '0 8px 28px rgba(35,32,28,.10)',
          display: 'flex',
          alignItems: 'center',
          boxSizing: 'border-box',
        }}
      >
        ...
        <div style={{ width: 240, padding: '0 22px' }}>
          ...
          <div style={{ font: '500 17px ' + SANS, color: v3 ? C.s900 : C.s400, marginTop: 3 }}>
            {v3 ? 'Jun 14, 2026' : 'Any date'}
          </div>
        </div>
        <div
          style={{
          ...
          return (
            <div
              ...
              style={{
                background: C.s0,
                borderRadius: 16,
                boxShadow: '0 2px 10px rgba(35,32,28,.06)',
                overflow: 'hidden',
                opacity: o,
  ...
  return (
    ...
    >
      {[
        ...
        <div
          key={i}
          style={{ padding: '0 18px', borderRight: i < 2 ? '1px solid ' + C.s200 : 'none' }}
        >
          <div
            style={{
              font: '600 9.5px ' + SANS,
  ...
  return (
    <div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          <Mark size={19} />
          <span style={{ font: '400 21px ' + SERIF, color: C.s900 }}>Orla</span>
        </div>
        <CompactSearchBar />
        <div
          style={{
      ...
      >
        ...
        >
          <span style={{ font: '400 20px ' + SERIF, color: C.s900 }}>18</span> photographers free
          that day
        </span>
      </div>
      <div
                ...
                <div style={{ position: 'absolute', left: 16, bottom: -17 }}>
                  <div
                    style={{
                      ...
                      background: i % 2 ? C.sage100 : C.clay100,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      font: '400 14px ' + SERIF,
                      color: i % 2 ? C.sageT : C.clayD,
                      boxSizing: 'border-box',
                    }}
                  >
                    {v[0].split(' ')[0][0] + (v[0].split(' ')[1] || ' ')[0]}
                  </div>
                </div>
              </div>
              <div style={{ padding: '22px 16px 16px' }}>
                <div style={{ font: '400 24px ' + SERIF, color: C.s900 }}>{v[0]}</div>
                <div style={{ font: '400 13.5px ' + SANS, color: C.s600, marginTop: 4 }}>
                  <b style={{ color: C.s700 }}>★ {v[1]}</b> ({v[2]}) · {v[3]}
                </div>
                <div style={{ marginTop: 11, display: 'flex', gap: 7 }}>
                  <Pill bg={C.s150} fg={C.s700}>
                    Photography
                  </Pill>
                  <Pill
                    bg={v[4] === 'Free Jun 14' ? C.sage50 : C.gold50}
                    fg={v[4] === 'Free Jun 14' ? C.sageT : C.goldT}
                  >
                    {v[4]}
                  </Pill>
                </div>
  ...
  return (
    <div>
      ...
      <div style={{ padding: '26px 60px' }}>
        ...
        <div style={{ font: '400 14px ' + SANS, color: C.s600, marginTop: 5 }}>
          June Harlow · Photography · Austin, TX
        </div>
      </div>
      <div
        style={{
      ...
      >
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          ...
          >
            {typed.slice(0, n)}
            <span style={{ color: C.clay }}>{n < typed.length && n > 0 ? '|' : ''}</span>
          </div>
          <div style={{ font: '400 12.5px ' + SANS, color: C.s600, marginTop: 7 }}>
            {n} / 1000 · vendors reply faster to specific requests
          </div>
        </div>
        <div
          style={{
        ...
        >
          <div
            style={{ width: 16, height: 16, borderRadius: '50%', background: C.sage, flex: 'none' }}
          />
          <span style={{ font: '400 14px ' + SANS, color: C.sageT }}>
            June has Jun 14 open. She has 48 hours to reply, and you'll get an email either way.
          </span>
        </div>
      </div>
      <div
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 14 }}>
          <CoverImg
            ...
            i={0}
            src={IMGS.vendors[0]}
            style={{ width: 68, flex: 'none', borderRadius: 10 }}
          />
          <div style={{ minWidth: 0, whiteSpace: 'nowrap' }}>
            <div style={{ font: '400 22px ' + SERIF, color: C.s900 }}>June Harlow</div>
            <div style={{ font: '400 13.5px ' + SANS, color: C.s600, marginTop: 3 }}>
              <b style={{ color: C.s700 }}>★ 4.9</b> (127)
            </div>
          </div>
        </div>
        <div style={{ height: 1, background: C.s200, margin: '18px 0' }} />
        <div style={{ font: '400 15px/1.6 ' + SANS, color: C.s700 }}>
          <b style={{ color: C.s900 }}>Full-day coverage</b>
          <br />8 hours · 2 shooters · gallery in 4 weeks
        </div>
        <div style={{ font: '400 30px ' + SERIF, color: C.s900, marginTop: 14 }}>From $1,450</div>
        <div style={{ marginTop: 22, transform: 'scale(' + press + ')' }}>
          <Btn style={{ width: '100%', textAlign: 'center', boxSizing: 'border-box' }}>
            Send request
          </Btn>
        </div>
        <div
    ...
    <div>
      ...
      <div
        style={{
          ...
          bottom: 0,
          background: C.s0,
          borderRight: '1px solid ' + C.s300,
          padding: '14px 0',
        }}
      >
        {[
          ['JH', 'June Harlow', 'Quote attached — $1,750', true],
          ['MC', 'Mesa Catering', 'Can you do 140 plated?', false],
          ['TL', 'Travis Loft', 'Walkthrough on the 3rd?', false],
        ].map((r, i) => (
          <div
            key={i}
            style={{
              ...
              alignItems: 'center',
              padding: '12px 16px',
              background: r[3] ? C.clay100 : 'transparent',
            }}
          >
            <Avatar t={r[0]} sage={i % 2 === 0} size={34} />
            <div style={{ minWidth: 0 }}>
              <div style={{ font: (r[3] ? '600 14px ' : '500 14px ') + SANS, color: C.s900 }}>
                {r[1]}
              </div>
              <div
                style={{
      ...
      >
        ...
        <div style={{ font: '400 13.5px ' + SANS, color: C.s600, marginTop: 5 }}>
          Wedding · 120 guests · Austin, TX
        </div>
        <div style={{ marginTop: 12 }}>
          {accept ? (
            <Pill bg={C.sage50} fg={C.sageT}>
              Confirmed
            </Pill>
          ) : (
            <Pill bg={C.steel50} fg={C.steel}>
              Quoted
            </Pill>
          )}
        </div>
        ...
        >
          <span>Second shooter</span>
          <b style={{ color: C.s900 }}>$300</b>
        </div>
        <div style={{ height: 1, background: C.s200, margin: '18px 0' }} />
        <div style={{ font: '400 34px ' + SERIF, color: C.s900 }}>$1,750</div>
        <div style={{ font: '400 13px/1.6 ' + SANS, color: C.s600, marginTop: 6 }}>
          Held until the event. June is paid the day after Jun 14.
        </div>
      </div>
      <div
        style={{
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div style={{ font: '400 12.5px ' + SANS, color: C.s600 }}>
        ...
        >
          ...
          >
            Hi Anjali — thanks for the request. Jun 14 is still open on my side.
          </div>
          <div
            style={{
              alignSelf: 'flex-end',
          ...
          >
            Wonderful. Outdoor ceremony, reception until 11 — could you send a price?
          </div>
          <div
            style={{
              alignSelf: 'flex-end',
          ...
          >
            Full day, ceremony at 5pm — 120 guests.
          </div>
          <div
            style={{
              alignSelf: 'flex-start',
          ...
          >
            Jun 14 is open. Sending you a quote now.
          </div>
          <div
            style={{
              alignSelf: 'flex-start',
            ...
            {[
              ...
              <div
                ...
                style={{
                  ...
                  gap: 20,
                  whiteSpace: 'nowrap',
                  font: '400 14.5px ' + SANS,
                  color: C.s700,
                  marginTop: i ? 9 : 0,
                }}
              >
                <span>{r[0]}</span>
                <b style={{ color: C.s900 }}>{r[1]}</b>
              </div>
            ))}
  ...
  return (
    <div
      style={{
        position: 'absolute',
        inset: 0,
        background: 'linear-gradient(150deg,#7A9468 0%,#5E7A4E 55%,#49613D 100%)',
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
        justifyContent: 'center',
    ...
    >
      ...
      <div
        style={{
          ...
          height: 84,
          borderRadius: '50%',
          border: '3px solid rgba(255,253,249,.85)',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
      ...
      >
        <div
          style={{
            width: 30,
            height: 16,
            borderLeft: '3px solid #FFFDF9',
            borderBottom: '3px solid #FFFDF9',
      ...
      >
        June Harlow · full-day coverage · $1,750 held until the event
      </div>
      <div
        style={{
          font: '400 14px ' + MONO,
  ...
  return (
    <div>
        ...
        {[
          ...
          <div
            ...
            style={{
              ...
              padding: '10px 12px',
              borderRadius: 9,
              font: (n[1] ? '600 14px ' : '500 14px ') + SANS,
              color: n[1] ? C.clayD : C.s700,
              background: n[1] ? C.clay100 : 'transparent',
              boxShadow: n[1] ? 'inset 3px 0 0 ' + C.clay : 'none',
            }}
          >
            {n[0]}
            {i === 1 && (
      ...
      >
        ...
        <div style={{ font: '400 14px ' + SANS, color: C.s600, marginTop: 4, marginBottom: 20 }}>
          One new request · 3 confirmed events this month
        </div>
        <div
          style={{
            width: 700,
            ...
            borderRadius: 18,
            padding: 22,
            boxShadow: '0 10px 30px rgba(35,32,28,.12)',
            transform: 'scale(' + (0.94 + 0.06 * arrive) + ')',
            opacity: arrive,
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {quoted ? (
              ...
              <Pill bg={C.clay100} fg={C.clayD}>
                Needs you
              </Pill>
            )}
            <span style={{ font: '400 13.5px ' + SANS, color: C.s600 }}>
              Received 2 minutes ago
            </span>
          </div>
          <div style={{ font: '400 26px ' + SERIF, color: C.s900, marginTop: 12 }}>
            Wedding · Jun 14, 2026 · 120 guests
          </div>
          <div style={{ font: '400 15px/1.6 ' + SANS, color: C.s700, marginTop: 8 }}>
            Anjali N. · Austin, TX · outdoor ceremony at 5pm, reception until 11.
          </div>
          <div
            style={{
              display: 'flex',
        ...
        <div style={{ display: 'flex', gap: 24, marginTop: 26 }}>
          ...
          >
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      background: on > 0.5 ? C.sage50 : C.s150,
                      border: '1px solid ' + (on > 0.5 ? '#C9D3BE' : C.s300),
                      transform: 'scale(' + (isTarget ? 0.9 + 0.1 * blocked : 1) + ')',
                    }}
                  >
                    <span style={{ font: '400 20px ' + SERIF, color: on > 0.5 ? C.sageT : C.s700 }}>
                      {d}
                    </span>
                    <span
                      style={{
            ...
            <div style={{ font: '400 13px ' + SANS, color: C.s600, marginTop: 14 }}>
              Accepted bookings block the date automatically.
            </div>
          </div>
          <div
            style={{
              ...
              boxSizing: 'border-box',
              opacity: payout,
              transform: 'translateY(' + (1 - payout) * 14 + 'px)',
            }}
          >
            ...
            >
              <div>
                <div style={{ font: '400 34px ' + SERIF, color: C.s900 }}>$1,750</div>
                <div style={{ font: '400 13.5px ' + SANS, color: C.s600, marginTop: 4 }}>
                  Anjali N. · Jun 14 wedding
                </div>
              </div>
              <div style={{ transform: 'translateY(' + (1 - payout) * 10 + 'px)' }}>
                <Pill bg={C.sage50} fg={C.sageT}>
                  Pays out Jun 15
                </Pill>
              </div>
            ...
            <div style={{ font: '400 14px/1.6 ' + SANS, color: C.s700 }}>
              Held by Orla until the event, then straight to your account —{' '}
              <b style={{ color: C.s900 }}>the day after Jun 14.</b>
            </div>
          </div>
        </div>
      </div>
...
function Close({ T, CUES, total }) {
  ...
  return (
    ...
    >
      <div style={{ transform: 'scale(' + mark + ')', marginBottom: 28 }}>
        <Mark size={92} gap={42} />
      </div>
      <div
        style={{
          font: '400 104px ' + SERIF,
          color: C.s900,
          lineHeight: 1,
          letterSpacing: '-.02em',
          opacity: word,
    ...
    >
      ...
      <div
        style={{
          width: 0,
          height: 0,
          borderLeft: '11px solid ' + C.s900,
          borderRight: '9px solid transparent',
          borderBottom: '16px solid transparent',
          borderTop: '0 solid transparent',
          transform: 'rotate(-14deg)',
          filter: 'drop-shadow(0 2px 4px rgba(35,32,28,.35))',
        }}
      />
      <div
        style={{
...
function OrlaIntro(props) {
  ...
  let click = 0;
  clicks.forEach((c) => {
    if (T >= c && T < c + 0.45) click = Math.max(click, 1 - (T - c) / 0.45);
  });
  const cursorOn =
    (showCursor ? 1 : 0) *
    MOTION.enter(0, 1, CUES.Search + 0.2, 0.5)(T) *
    (T > CUES.Confirmed - 0.4 && T < CUES.VendorDay + 1.4 ? 0 : 1) *
    (T > CUES.Close - 0.6 ? MOTION.draw(1, 0, CUES.Close - 0.6, 0.5)(T) : 1);
  ...
  return (
    <div
      data-screen-label={'Introducing Orla · t=' + Math.floor(T) + 's'}
      style={{
        position: 'absolute',
        inset: 0,
        background: 'linear-gradient(160deg,#EFEAE1 0%,#E9E4DA 46%,#E2DCD1 100%)',
        overflow: 'hidden',
        fontFamily: SANS,
      }}
    >
      <div
        style={{
          ...
          borderRadius: '50%',
          background:
            'radial-gradient(circle at 40% 40%, rgba(180,85,47,.10), rgba(180,85,47,0) 70%)',
        }}
      />
      <div
        style={{
          ...
          borderRadius: '50%',
          background:
            'radial-gradient(circle at 50% 50%, rgba(94,107,79,.10), rgba(94,107,79,0) 70%)',
        }}
      />
      ...
      <AudioTrack
        ...
        T={T}
        playing={playing}
        volume={AUDIO.musicVolume == null ? 0.65 : AUDIO.musicVolume}
        total={total}
      />
      <AudioTrack
        src={AUDIO.voice}
        T={T}
        playing={playing}
        volume={AUDIO.voiceVolume == null ? 1 : AUDIO.voiceVolume}
        total={total}
      />

      <Shot from={0} to={CUES.Search + 0.4}>
        <Opening T={T} CUES={CUES} />
      </Shot>
      <Shot from={CUES.Scatter} to={CUES.Bridge + 1.4}>
        {FRAGS.map((f, i) => (
          <FragCard key={i} f={f} T={T} CUES={CUES} />
        ))}
      </Shot>
      <Shot from={CUES.Bridge - 0.2} to={CUES.Search + 0.6}>
        <Bridge T={T} CUES={CUES} />
      </Shot>
        ...
        <div
          style={{
            ...
            transform:
              ...
              (camY + devLift) +
              'px) scale(' +
              sc * (0.94 + 0.06 * devIn) * (0.86 + 0.14 * devOut) +
              ')',
          }}
        >
          <div
            style={{
              ...
              borderRadius: 18,
              overflow: 'hidden',
              boxShadow: '0 30px 90px rgba(35,32,28,.22), 0 2px 0 rgba(255,255,255,.5) inset',
            }}
          >
            <div
              style={{
            ...
            >
              {[C.s400, C.s400, C.s400].map((d, i) => (
                <div
                  key={i}
                  style={{ width: 11, height: 11, borderRadius: '50%', background: d }}
                />
              ))}
              <div
                style={{
            ...
            >
              ...
              <div
                style={{
                  position: 'absolute',
                  inset: 0,
                  transform: 'translateY(' + -flip * 100 + '%)',
                }}
              >
                <div style={{ position: 'absolute', inset: 0 }}>
                  <Shot from={CUES.Search - 0.8} to={CUES.Results}>
                    <SearchScreen T={T} CUES={CUES} />
                  </Shot>
                  <Shot from={CUES.Results} to={CUES.Request}>
                    <ResultsScreen T={T} CUES={CUES} />
                  </Shot>
                  <Shot from={CUES.Request} to={CUES.Quote}>
                    <RequestScreen T={T} CUES={CUES} />
                  </Shot>
                  <Shot from={CUES.Quote} to={CUES.Confirmed}>
                    <QuoteScreen T={T} CUES={CUES} />
                  </Shot>
                  <Shot from={CUES.Confirmed} to={CUES.VendorDay}>
                    <ConfirmedScreen T={T} CUES={CUES} />
                  </Shot>
                </div>
              </div>
              {/* vendor layer */}
              <div
                style={{
                  position: 'absolute',
                  inset: 0,
                  transform: 'translateY(' + (1 - flip) * 100 + '%)',
                }}
              >
                <div style={{ position: 'absolute', inset: 0 }}>
                  <Shot from={CUES.Flip} to={CUES.Close + 1}>
                    <VendorScreen T={T} CUES={CUES} />
                  </Shot>
                </div>
              </div>
            </div>
      ...
      <Shot from={CUES.Close - 0.9} to={total + 1}>
        <Close T={T} CUES={CUES} total={total} />
      </Shot>
      ...
      {showCaptions && (
        <Captions
          style={{
            ...
            borderRadius: 999,
            textShadow: 'none',
            boxShadow: '0 10px 34px rgba(35,32,28,.16)',
            border: '1px solid ' + C.s300,
          }}
          items={[
            {
              at: CUES.Scatter + 0.6,
              until: CUES.Bridge + 0.35,
              text: 'Booking event vendors used to mean fifteen tabs and four DMs.',
            },
            {
              at: CUES.Bridge + 0.55,
              until: CUES.Search + 0.6,
              text: 'Now — book your vendors without the back-and-forth.',
            },
            {
              at: CUES.Search + 0.9,
              until: CUES.Search + 5.7,
              text: 'Start with what you need, where, and when.',
            },
            {
              at: CUES.Results + 0.8,
              until: CUES.Results + 5.6,
              text: 'Clear prices. Open calendars.',
            },
            {
              at: CUES.Request + 0.7,
              until: CUES.Request + 5.0,
              text: 'Send one request — nothing is charged yet.',
            },
            {
              at: CUES.Quote + 1.0,
              until: CUES.Quote + 5.8,
              text: 'Pay securely once the date is locked in.',
            },
            {
              at: CUES.Flip + 0.5,
              until: CUES.VendorDay + 1.4,
              text: 'The same booking, from the vendor’s side.',
            },
            {
              at: CUES.VendorDay + 1.9,
              until: CUES.VendorDay + 7.6,
              text: 'Quote, block the date, get paid the day after.',
            },
          ]}
        />
      )}

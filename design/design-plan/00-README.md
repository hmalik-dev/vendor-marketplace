...
| `/sign-up/customer-details`   | **Exempt — no frame; the ticket's own frame was canceled.** The mandatory name step every customer clears once, right after `/accept-terms` (VEN-642). `#643` would have drawn a dedicated frame and was canceled, ruled not a blocker; built instead directly against `accept-terms-screen.tsx`'s existing composition — same `max-w-[700px]` panel, label-then-heading rhythm and button/helper-text treatment, the same weight class `/sign-up/vendor-details` is. |

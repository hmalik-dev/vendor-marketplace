...
| Route                         | Ruling                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/sign-in`                    | **Exempt.** VEN-470 adds a `Forgot password?` link under the password field that frame `12` does not draw (ruled on the ticket: the reset screens reuse the sign-in card); a parity pass reading it is looking at this ruling. The form is auth-hosted and we do not control its markup. The surface around it — the marketing panel — **is** framed, by `12 Sign up`, and that panel is on the parity gate.                                                                                                                                      |
| `/suspended`                  | **Exempt.** A dead end for a banned account, reached by no navigation. `#15` has now landed the admin tooling behind suspensions and did **not** draw it: the console is where a suspension is _made_, and the notice a suspended account sees is a one-line dead end, not a screen the operator works in.                                                                                                                                                                                                                                        |
...
| `/admin/vendors`              | **Framed.** `13 Admin` draws exactly this table. The frame draws it with the saved filter on, so the state to compare is `/admin/vendors?status=review`.                                                                                                                                                                                                                                                                                                                                                                                          |
| `/admin`                      | **Exempt — derived.** The Overview. `22-admin.md` specifies it in prose ("four metric cards, then line charts … revenue gold, bookings clay, users steel, completion sage") and the design bundle draws no frame for it. It is built to that prose and to frame `13`'s shell, which **is** framed.                                                                                                                                                                                                                                                |
| `/admin/customers`            | **Exempt — derived.** The same table as `13 Admin`, over a different projection. The frame defines the table; a second drawing of it per entity would be four copies of one composition, and a divergence between them would be the defect rather than the fix.                                                                                                                                                                                                                                                                                   |
| `/admin/bookings`             | **Exempt — derived.** As `/admin/customers`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `/forgot-password`            | **Exempt — derived.** The password-reset request (VEN-470, ruled on the ticket 2026-09-20: no frame; the screens reuse the sign-in card). Frame `12`'s sign-in split screen with one email field and one primary.                                                                                                                                                                                                                                                                                                                                 |
| `/reset-password`             | **Exempt — derived.** The emailed code and the new password (VEN-470). Frame `12`'s sign-in split screen; the code field is the sign-up verify step's and the password field is sign-up's, rules unchanged.                                                                                                                                                                                                                                                                                                                                       |
| `/accept-terms`               | **Frame `40` (customer welcome), `design/delta-customer-name-collection/`** (VEN-744): one centred column with the logo, `Welcome to {brand}`, `You're joining as {role}.`, one full-width `Continue` and the consent line under it. The frame labels the route `/sign-up/accept-terms`; the route stays `/accept-terms`. The new-Terms-version screen (`explicitTickRequired`) is not in that frame: it stays derived from frame `32`'s unaccepted state (`#429`) — the clipped document card, the unticked checkbox, the disabled primary.      |
...
| `/admin/vendors/[vendorId]`   | **Exempt — derived.** One vendor's whole record (VEN-380): Stripe, packages, portfolio, availability locks and notifications sent, with identity and actions in the right column. Frame `13` draws no detail; it is built to the admin delta's Pattern B (`design/delta-admin/`, frame `B Admin vendor detail`), the same written rule `/admin/users/[userId]` follows, and entered from `View` in a vendor row's menu.                                                                                                                           |
| `/admin/bookings/[bookingId]` | **Exempt — derived.** One booking's money story (VEN-399): amounts, payout state and attempts, refund, cancellation and dispute in one card, in event order, with identity and links to both parties on the right. Built to the admin delta's Pattern B card order for `/admin/bookings/[id]` (`design/delta-admin/`), inside frame `13`'s shell, and entered from a booking row's event date or a closure refusal.                                                                                                                               |
...
| `/admin/operators`            | **Exempt — derived.** Who can sign in to the console (VEN-506): name, email, since and granted by, with Grant and Revoke behind the step-up confirm. No frame draws it; it is frame `13`'s table inside its shell, reached by an `Operators` row appended beside `Settings`, the same rule `/admin/vendor-applications` follows.                                                                                                                                                                                                                  |
| `/vendors/apply`              | **Exempt — redirect only.** The old home of the vendor gate's application form (VEN-406), kept so links keep working: signed out goes to `/sign-up?role=vendor`, a verified vendor with incomplete details to `/sign-up/vendor-details`, one with complete details to `/waitlist`. Draws nothing.                                                                                                                                                                                                                                                 |
| `/sign-up/vendor-details`     | **Framed by the waitlist delta.** The vendor waitlist's details step, frame `36` of `design/delta-waitlist/` on frame `12 Sign up`'s split screen: the verified email as display, business name, category, city, state and an optional link to their work, one `Add me to the waitlist` button, no back link and no sign-out control. Vendor only, unreachable until the email is verified (VEN-512).                                                                                                                                             |
| `/sign-up/customer-details`   | **Frames `41` and `41b` (name, and its error, saving and failed-save states), `design/delta-customer-name-collection/`** (VEN-744). The mandatory name step every customer clears once, right after `/accept-terms` (VEN-642): the same shell as frame `40`, two fields, `Continue`, and a centred `Sign out` link. The frame labels the route `/sign-up/name`; the route stays `/sign-up/customer-details`. The frame's SSO prefill is not built: a fresh account carries only the sign-up form's placeholder, which is never offered as a name. |
| `/waitlist`                   | **Framed by the waitlist delta.** The terminal screen, frame `37` of `design/delta-waitlist/`: no form, no button, one `Back to {BRAND_NAME}` link that also signs the person out. A waitlisted vendor has a login and no app account yet, so every route that needs an account redirects here until the invite is accepted (VEN-512).                                                                                                                                                                                                            |
...
| `/account/settings`           | **Exempt — derived.** The account settings area (VEN-677, ruled by the account holder 2026-09-23): reached from `Account settings` in the account menu, for every role, the admin console's header included. A centered list, one row per setting (label, current value, link), the same for every role and built from the row vocabulary of the admin and vendor navs (VEN-703); today `Your name` and `Password`, and a later setting is one more row.                                                                                          |
| `/account/settings/name`      | **Exempt — derived.** The `Your name` row's page (VEN-703): first and last name for every role, in frame `12`'s field and button vocabulary; saving returns to the list with a confirmation banner.                                                                                                                                                                                                                                                                                                                                               |
| `/account/settings/password`  | **Exempt — derived.** The `Password` row's page (VEN-677, VEN-703): current, new and confirm in frame `12`'s field and button vocabulary, the same basis `/reset-password` uses; success goes to the role's home.                                                                                                                                                                                                                                                                                                                                 |
| `/account/settings/sessions`  | **Exempt — derived.** The `Where you're signed in` row's page (VEN-681, VEN-703): one row per device (label, last active, `This device` on the caller's own) with a `Sign out` per other device and `Sign out all other devices`, in the account settings list's row vocabulary and the existing `Button` and `Banner`; no IP address is shown.                                                                                                                                                                                                   |
| `/account/settings/close`     | **Exempt — derived.** The `Close account` row's page (VEN-680), customers and vendors only: what is kept and what goes, the upcoming bookings that refuse it (D39), then an emailed code and the address typed back, in frame `12`'s field and button vocabulary; neutral, not red (`40-states.md`).                                                                                                                                                                                                                                              |
| `/account/closed`             | **Exempt — derived.** The farewell after a closure (VEN-680): a heading, one sentence and a link home; public, because the session is already gone.                                                                                                                                                                                                                                                                                                                                                                                               |
...
**derived** from it. `/admin/activity` is the eighth, added by `#434`, and
`/admin/cases` with its detail the ninth and tenth, added by `#431`; the rail
...
the rail gains that no frame draws. `/admin/vendors/[vendorId]` is the
thirteenth, added by VEN-380, and like the account record the rail does not
...
rule in a screen file — not that nobody got round to drawing it. It is available
only where drawing the route again would produce a second copy of a composition
the bundle already fixes; anything with a layout of its own still needs a frame.
Frame `13` is on the parity gate, and the derived routes inherit its verdict.
...
in the sweep ledger by `#319`). **`13 Admin` now has a route too** —
`/admin/vendors`, built by `#15` — so the reverse gap is closed.
